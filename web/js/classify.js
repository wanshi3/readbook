...
$(function (){
	var forData={"Theme":"novel","Genre":"romance","param":""}
	...
	$.ajax({
		// URL
		url: 'http://localhost:8080/web_war_exploded/com/demo/controller/book/classify',
		// 参数
		data: forData,
		// 请求类型
		type: 'POST',
		...
		success: function (data) {
			...
			let elements3 = document.querySelectorAll('.f3');
			...
			elements3.forEach((element,index) => {
				if(index<data.rows.length)
				{
					element.innerText = "￥"+data.rows[index].price
				}
			...
			elements4.forEach((element, index) => {
				if (index < data.rows.length) {
					element.href = "bookcontain.html?BookID=" + data.rows[index].bookID;
				}
			});
			//window.history.back();
