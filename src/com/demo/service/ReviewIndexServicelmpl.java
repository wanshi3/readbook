...
public class ReviewIndexServicelmpl {
    boolean success;//操作成功与否
    String msg;//返回的结果信息
    int total;//返回记录数
    JsonData jd;//将数据转换为指定JSON格式的对象，并返回给Controller层
    public JsonData select(Reviewbook obj) {
        Connection conn = DButil.getConnection();//数据库连接
        ReviewbookDAOImpl bo = new ReviewbookDAOImpl(conn);//业务层对象，用于调用DAO层方法
        try{
            ArrayList<Reviewbook> rows = new ArrayList<Reviewbook>();
            //返回记录集
    ...
    public JsonData insert(Reviewbook obj){
        Connection conn = DButil.getConnection();//数据库连接
        ReviewbookDAOImpl bookBO = new ReviewbookDAOImpl(conn);//业务层对象，用于调用DAO层方法
        try{
